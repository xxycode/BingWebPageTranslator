...
: node.textContent;
    ...
    notifyMediator(command, payload) {
        ...
        if (command === "translate") {
            console.log('attrId: ' + payload.attrId);
            console.log('text: ' + payload.text);
            this.requestTranslate(payload);
        }
    }
...
: this.UI_UPDATE_INTERVAL);
    ...
    *ancestors(node) {
    ...
    requestTranslate(payload) {
        window.webkit.messageHandlers.translationHandler.postMessage(payload);
    }
}

window.translator = new InPageTranslation();
